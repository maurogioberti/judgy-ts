...
import { MoonshotProvider } from "../src/index.js";
...
describe("MoonshotProvider", () => {
  it("sends the correct request", async () => {
    ...
    const provider = new MoonshotProvider({
      apiKey: "moonshot-test-key-12345",
      fetch: fakeFetch.handler
    });

    await provider.complete(new LlmRequest({ prompt: "What is 2+2?" }));

    expect(fakeFetch.lastInput?.toString()).toBe("https://api.moonshot.ai/v1/chat/completions");
    expect(fakeFetch.lastInit?.method).toBe("POST");

    const headers = new Headers(fakeFetch.lastInit?.headers);
    expect(headers.get("authorization")).toBe("Bearer moonshot-test-key-12345");
  });
  ...
  it("parses the response correctly", async () => {
    ...
    expect(response.providerName).toBe("Moonshot");
  ...
  it("throws on HTTP errors", async () => {
    ...
    await expect(provider.complete(new LlmRequest({ prompt: "What is 2+2?" }))).rejects.toThrow(
      "Moonshot request failed with status 500."
    );
  });
  ...
  it("throws when the response payload is empty", async () => {
    ...
    await expect(provider.complete(new LlmRequest({ prompt: "What is 2+2?" }))).rejects.toThrow(
      "Moonshot returned an empty response payload."
    );
  });

  it("validates constructor options", () => {
    expect(() => new MoonshotProvider(undefined as never)).toThrow("Options are required.");
    expect(() => new MoonshotProvider({ apiKey: "" })).toThrow("ApiKey cannot be null or whitespace.");
  });
