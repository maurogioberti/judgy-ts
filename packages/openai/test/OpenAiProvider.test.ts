...
describe("OpenAiProvider", () => {
  it("sends the correct request", async () => {
    ...
    expect(fakeFetch.lastInput?.toString()).toBe("https://api.openai.com/v1/chat/completions");
    expect(fakeFetch.lastInit?.method).toBe("POST");

    const headers = new Headers(fakeFetch.lastInit?.headers);
    expect(headers.get("authorization")).toBe("Bearer sk-test-key-12345");
  });
  ...
  it("validates constructor options", () => {
    expect(() => new OpenAiProvider(undefined as never)).toThrow("Options are required.");
    expect(() => new OpenAiProvider({ apiKey: "" })).toThrow("ApiKey cannot be null or whitespace.");
  });
