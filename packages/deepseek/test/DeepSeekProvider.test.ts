...
describe("DeepSeekProvider", () => {
  it("sends the correct request", async () => {
    ...
    expect(fakeFetch.lastInput?.toString()).toBe("https://api.deepseek.com/chat/completions");
    expect(fakeFetch.lastInit?.method).toBe("POST");

    const headers = new Headers(fakeFetch.lastInit?.headers);
    expect(headers.get("authorization")).toBe("Bearer deepseek-test-key-12345");
  });
  ...
  it("validates constructor options", () => {
    expect(() => new DeepSeekProvider(undefined as never)).toThrow("Options are required.");
    expect(() => new DeepSeekProvider({ apiKey: "" })).toThrow("ApiKey cannot be null or whitespace.");
  });
